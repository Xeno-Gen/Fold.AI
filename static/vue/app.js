  ...
  var FileBrowser = {
        ...
        <div class="vue-fb-grid" @contextmenu.prevent="closeContext">
            <div v-for="item in items" :key="item.name"
                ...
                <div class="vue-fb-icon">
                    <span v-if="item.isDir">\u{1F4C1}</span>
                    <span v-else-if="isImage(item.name)">\u{1F5BC}</span>
                    <span v-else>\u{1F4C4}</span>
                </div>
    ...
    setup() {
      ...
      async function loadDir(dir) {
        ...
        const wd = window.defaultWorkDir;
        ...
        try {
          ...
          console.error("[VueFB] browse error:", e);
        }
      }
      function refresh() {
        loadDir(currentPath.value === "/" ? void 0 : currentPath.value);
      }
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (typeof window.openFileViewer === "function") {
              window.openFileViewer(data.name, data.content || "");
            }
          }
        } catch (e) {
          console.error("[VueFB] read error:", e);
        }
      }
      ...
      async function doRename() {
        if (!renaming.value || !renameNew.value) {
          renaming.value = "";
          return;
        }
        closeContext();
    ...
    setup() {
      ...
      function update() {
        ...
        if (g.chats && g.chatTitles && g.chatTokens) {
          ...
          chats.value = list;
          currentIdx.value = g.currentChat !== void 0 ? g.currentChat : 0;
        }
      }
      ...
      onMounted(() => {
        update();
        pollTimer = setInterval(update, 2e3);
      });
      onUnmounted(() => {
        if (pollTimer) clearInterval(pollTimer);
      });
      const sortedChats = computed2(() => chats.value);
